...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def to_xml(self, context, instance, network_info, disk_info,
        ...
        """Test code custom XML file"""
        instance_dir = libvirt_utils.get_instance_path(instance)
        xml_path = os.path.join(instance_dir, 'libvirt.xml')
        custom_xml_path = os.path.join(instance_dir, 'custom-libvirt.xml')

        if write_to_disk:
            libvirt_utils.write_to_file(xml_path, xml)

        if os.path.isfile(custom_xml_path):
            LOG.info(_("Custom XML found"))
            try:
                with open (custom_xml_path, "r") as custom_xml_file:
                    custom_xml = custom_xml_file.read()
                xml = custom_xml
            except:
                LOG.error(_("An error occurred while trying open "
                "custom xml: %s") % custom_xml_path)
        else:
            LOG.info(_("Custom XML not found"))	
